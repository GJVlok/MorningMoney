            ...
            ft.Container(
                ft.Column([
                    ft.ListTile(
                        ...
                        title=ft.Text("Edit"),
                        on_click=lambda _: (
                            page.run_task(edit_transaction_dialog, page, t, refresh_all),
                    ...
                    ft.ListTile(
                        ...
                        on_click=lambda _: (
                            page.run_task(delete_transaction, page, t, refresh_all),
                            setattr(bs, "open", False),
                            page.update()
                        )
                    ),
...
def transaction_tile(transaction: Transaction, page: ft.Page, refresh_all, running_balance: float = None, variant="desktop") -> ft.Control:
    """
    ...
    title_area = ft.Column([
        ...
        ft.Text(transaction.description or transaction.date.strftime("%d %b %Y"), color="grey", no_wrap=False)
    ], spacing=2, expand=1)

    base = ft.ListTile(
        leading=ft.Icon(ft.Icons.RECEIPT, color="#ff0066"),
        title=title_area,
        trailing=ft.Container(trailing_content, expand=0, width=120),
        dense=False
    )
    ...
    if variant == "desktop" or variant == "web":
        return ft.Container(
            ...
            ], alignment="spaceBetween", expand=True, vertical_alignment="center"),
    ...
    return ft.Dismissible(
        content=ft.Card(
            elevation=4,
            content=ft.Container(base, padding=12, border_radius=10, expand=True, on_click=_mobile_transaction_menu(transaction, page, refresh_all)),
            margin=8,
            color="#1e1e2e",
        ),
