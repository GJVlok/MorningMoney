# ui/sections/mobile/investments.py
import flet as ft
...
class InvestmentsTab(ft.Column):
    ...
    def _build(self):
        self.controls = [
            ft.Text("Investments", size=24, weight="bold"),
            self.container,
        ]
    ...
    async def refresh(self):
        ...
        for inv in investments:
            self.container.controls.append(
                investment_card(inv, self.page, self.refresh_all, variant="mobile")
            )

        self.container.controls.append(
            investment_form(self.page, self.refresh_all, variant="mobile")
        )
